...
import { ChevronLeft, Building2, MapPin, Briefcase, CalendarDays, Pencil, MessageSquare } from "lucide-react";
...
export default async function MemberProfileView({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="min-h-[100dvh] px-6 pb-10 pt-6" style={{ background: "var(--ground)" }}>
      ...
      <div className="mt-5 flex flex-col items-center rounded-2xl border border-[--border] bg-surface p-6 text-center shadow-card">
        ...
        {isMe ? (
          ...
          <Link href={`/app/messages/${member.id}`} className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-600">
