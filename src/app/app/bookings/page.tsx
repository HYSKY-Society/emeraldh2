...
export default async function BookingsPage() {
  ...
  return (
    <main className="min-h-[100dvh] px-6 pb-10 pt-6" style={{ background: "var(--ground)" }}>
      ...
      {bookings.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[--border] bg-white px-4 py-10 text-center text-sm text-ink-muted">
          No bookings yet. <Link href="/app/find" className="font-semibold text-brand-600">Find a station</Link> to reserve fuel.
        </div>
      ) : (
        <div className="flex flex-col gap-2.5">
          {bookings.map((b) => (
            <Link key={b.id} href={`/app/bookings/${b.id}`} className="flex items-center gap-3 rounded-xl border border-[--border] bg-white p-4 shadow-card transition hover:shadow-md">
