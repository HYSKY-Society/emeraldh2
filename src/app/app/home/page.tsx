...
export default async function HomePage() {
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col" style={{ background: "var(--ground)" }}>
      {/* header (mobile only — desktop uses the sidebar brand) */}
      <header className="flex items-center justify-between px-6 pb-3 pt-6 lg:hidden">
      ...
      <div className="flex-1 px-6 pb-6 lg:pt-8">
        {/* greeting */}
        ...
        {/* dashboard — single column on mobile, two-up on desktop */}
        <div className="mt-5 lg:grid lg:grid-cols-3 lg:items-start lg:gap-5">
          {/* main column */}
          <div className="lg:col-span-2 lg:space-y-4">
            {/* wallet */}
            <div className="overflow-hidden rounded-2xl p-5 text-white shadow-lg" style={{ background: "linear-gradient(135deg,#0b3f26,#0b8a4b)" }}>
            ...
            {/* primary CTA */}
            <Link href="/app/find" className="mt-4 flex items-center justify-between rounded-2xl bg-surface p-5 shadow-card ring-1 ring-brand-100 transition hover:shadow-md lg:mt-0">
          ...
          <div className="mt-4 lg:mt-0 lg:space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 lg:grid-cols-1">
              {[
                ...
                <Link key={t.label} href={t.href} className="flex flex-col items-center gap-2 rounded-xl border border-[--border] bg-surface px-2 py-4 text-center shadow-card lg:flex-row lg:justify-start lg:gap-3 lg:px-4 lg:text-left">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-brand-50 text-brand-600"><t.icon size={17} /></span>
                  <span className="text-xs font-medium text-ink-soft lg:text-sm">{t.label}</span>
                </Link>
              ))}
            </div>

            {/* notifications */}
            <div className="mt-4 lg:mt-0"><PushToggle /></div>
          </div>
        </div>
